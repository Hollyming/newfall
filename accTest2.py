...
#特殊数据集测试需要
data_path1 = "./special_data/falldown/"
data_path2 = "./special_data/sit/"
trueNum = 0
falseNum = 0
...
for file_name in os.listdir(data_path1):
    with open(os.path.join(data_path1, file_name), 'rb') as f:
        data = pickle.load(f)
        skels, deps, label = data['skel'], data['dep'], data['label']
...
#     ts_pinfo = time.time()
    ...
    while (1):
        ...
        #raw_frame为深度图和红外图的组合，480*1280*2;ret为bool值,若读取到帧则为True
        if not ret:#若未读取到帧，则跳过
            time.sleep(0.1)
            continue
        ...
        if bgflag :
            ...
            depnb = cv2.convertScaleAbs(img_dep, None, 1 / 16)  #convertScaleAbs函数用于线性变换，此处为1/16
            depnb = cv2.merge([depnb] * 3)                      #merge函数用于合并通道，此处为3通道
            ...
            if ydiff is not None:
                if ydiff > 200:
                    ...
                    fine_num = 0
            else:                   #骨架不显示的区域，用bndbox判断
                if cbk > 1.5:
                    fine_num = fine_num + 1
                else:
                    fine_num = 0

            if fine_num == 5:          #连续5帧ydiff大于200，判断为跌倒结束
                flag = 0
                fine_num = 0
                isFallDown = 0    
